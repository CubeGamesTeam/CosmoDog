    ...
    public class AutoLocalizationMasse : EditorWindow
    {
        ...
        private void OnGUI()
        {
            if (GameObject.FindObjectOfType<YandexGame>().infoYG.translateMethod == InfoYG.TranslateMethod.AutoLocalization)
            {
                ...
                if (GUILayout.Button("Search for all objects on the scene by type Text/TextMesh/TextMeshPro", GUILayout.Height(30)))
                {
                    ...
                    foreach (TMP_Text obj in SceneAsset.FindObjectsOfType<TMP_Text>())
                    {
                        objectsTranlate.Add(obj.gameObject);
                    }
                }
